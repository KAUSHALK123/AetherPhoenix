...
from shared.contracts.feedback import PlannerFeedback, CapabilityFailureInfo
...
def test_planner_normal_task_decomposition(planner_agent):
    ...
    assert "sales" in response.reply.lower() or "presentation" in response.reply.lower() or "tasks" in response.reply.lower()
...
def test_planner_followup_request_after_clarification(planner_agent):
    """Test follow-up request after clarification restores active session prompt."""
    ...
    assert response.status == "ready"
    assert response.action == "execute_plan"
    assert session_id not in planner_agent.active_sessions  # session cleared after completion
...
def test_planner_unsupported_capability(planner_agent):
    ...
    with patch.object(planner_agent.capability_engine, 'discover_capabilities') as mock_discover:
