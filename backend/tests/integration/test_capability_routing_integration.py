...
def test_routing_ipconfig(decomposer, planner_agent):
    ...
    req = PlannerRequest(
        session_id=str(workflow_id), message="Run ipconfig on my laptop.", context={}
    )
...
def test_routing_what_is_my_ip_address(decomposer, planner_agent):
    """TEST 2b: 'what is my ip address' must route to terminal_tool / POWERSHELL."""
    workflow_id = uuid4()
    plan = decomposer.decompose_goal("what is my ip address", workflow_id)
    assert len(plan.tasks) >= 2
    leaf_tasks = [t for t in plan.tasks if t.required_tool]
    assert len(leaf_tasks) > 0
    assert leaf_tasks[0].required_tool == "terminal_tool"
...
def test_routing_open_notepad(decomposer, planner_agent):
    ...
    req = PlannerRequest(
        session_id=str(workflow_id), message="Open Notepad.", context={}
    )
    res = planner_agent.process_request(req)
    assert res.status == "ready"
    reply_data = json.loads(res.reply)
...
def test_routing_open_downloads(decomposer, planner_agent):
    ...
    req = PlannerRequest(
        session_id=str(workflow_id), message="Open my Downloads folder.", context={}
    )
    res = planner_agent.process_request(req)
    assert res.status == "ready"
    reply_data = json.loads(res.reply)
...
def test_routing_ppt_generation(decomposer, planner_agent):
    ...
    leaf_tasks = [
        t
        for t in plan.tasks
        if t.category == TaskCategory.PPT_GENERATION and t.required_tool
    ]
    assert len(leaf_tasks) > 0
...
def test_routing_ocr_extraction(decomposer, planner_agent):
    ...
    workflow_id = uuid4()
    plan = decomposer.decompose_goal(
        "Extract the text from this uploaded image.", workflow_id
    )
    assert len(plan.tasks) >= 2
    leaf_tasks = [
        t for t in plan.tasks if t.category == TaskCategory.OCR and t.required_tool
    ]
    assert len(leaf_tasks) > 0
