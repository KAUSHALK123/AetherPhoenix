...
def executor():
    return PowerShellExecutor(
        permission_manager=MockPermissionManager(should_approve=True)
    )
...
def denied_executor():
    return PowerShellExecutor(
        permission_manager=MockPermissionManager(should_approve=False)
    )
...
async def test_timeout_handling(executor):
    ...
    # The exact exit code after kill might vary by OS,
    # but timeout_occurred is the key flag.
...
async def test_capture_stderr(executor):
    ...
    )  # Write-Error sets $LASTEXITCODE in some PS versions but not always.
