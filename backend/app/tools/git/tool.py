...
class GitToolInput(BaseModel):
    ...
    operation: str = Field(
        ...,
        description="The git operation to perform: status, branches, history, checkout, stage, commit, diff, pull, push."
    )
    branch: Optional[str] = Field(None, description="The branch name for checkout.")
    create_branch: Optional[bool] = Field(False, description="Whether to create the branch if it doesn't exist.")
    files: Optional[List[str]] = Field(None, description="Files to stage or check diff for.")
    message: Optional[str] = Field(None, description="Commit message.")
    limit: Optional[int] = Field(10, description="Limit for git history.")
    working_directory: Optional[str] = Field(None, description="Working directory for the git repository. Defaults to current directory.")
