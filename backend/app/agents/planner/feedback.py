        ...
        if feedback.replanning_context:
            replanning_event = Event(
                ...
                payload={
                    ...
                    "workflow_id": str(state.metadata.workflow_id),
                    "session_id": str(
                        getattr(state.metadata, "session_id", None)
                        or getattr(state.metadata, "conversation_id", None)
                        or state.metadata.workflow_id
                    ),
                    "goal": state.metadata.goal,
                    "failed_task_id": (
                        str(failure_report.task_id) if failure_report else None
                    ),
                },
            )
