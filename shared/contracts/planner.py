...
class Goal(BaseModel):
    ...
    parent_id: str | None = Field(None, description="Optional ID of the parent goal.")
...
class PlannerResponse(BaseModel):
    ...
    action: str | None = Field(None, description="The next action if applicable.")
...
class TaskDecompositionPlan(BaseModel):
    ...
    estimated_total_duration_seconds: int | None = Field(
        default=None, description="Estimated total duration in seconds."
    )
    unsupported_capabilities: list[str] = Field(
        default_factory=list, description="Capabilities identified but not supported."
    )
...
class PlanMetadata(BaseModel):
    ...
    planner_model: str = Field(
        default="gemini-pro", description="LLM used for planning"
    )
    execution_mode: str = Field(
        default="ASSISTED", description="Execution mode requested"
    )
...
class PlannerOutput(BaseModel):
    ...
    workflow_spec: str = Field(
        description="High-level description of the workflow logic"
    )
    tasks: list[Task] = Field(
        default_factory=list, description="Ordered/unordered list of task executions"
    )
    ...
    execution_summary: str = Field(
        default="", description="Summary of the execution plan"
    )
    parallel_groups: list[list[UUID]] = Field(
    ...
    def validate_acyclic_graph(
        ...
        def visit(node: UUID):
            if node in path:
                raise ValueError(
                    f"Cycle detected in dependency graph involving task {node}"
                )
            if node in visited:
                return
    ...
    def validate_tasks_match_graph(self) -> "PlannerOutput":
        ...
        for task_id, dependencies in self.dependency_graph.items():
            if task_id not in task_ids:
                raise ValueError(
                    f"Task {task_id} in dependency graph is missing from tasks list"
                )
    ...
    def validate_tasks_tools(self) -> "PlannerOutput":
        ...
        for task in self.tasks:
            if task.task_type == TaskType.LEAF and not task.required_tool:
                raise ValueError(
                    f"LEAF task {task.task_id} must have a non-empty required_tool"
                )
            if task.task_type == TaskType.PHASE and task.required_tool:
                raise ValueError(
                    f"PHASE task {task.task_id} must have an empty required_tool"
                )
        return self
